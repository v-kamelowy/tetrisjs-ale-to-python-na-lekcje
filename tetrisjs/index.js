define(["require", "exports"], function (require, exports) {
    ...
    exports.MyApp = exports.GamePadState = void 0;
    var GAME_MODE;
    (function (GAME_MODE) {
    ...
    var MyApp = /** @class */ (function () {
        ...
        MyApp.prototype.newPaint = function () {
            ...
            const mainColor = "#e070ff"
            //const mainShadowColor = "#a040ff";
            const secondColor = "#335";
            const bgColor = "#112";
            $("[tetris-block]").toArray().forEach(function (element) {
                ...
                if (_this.gameMatrixBuffer[y][x] == 8 || _this.gameMatrix[y][x] == 8) {
                    ...
                    element.style["background-image"] = `linear-gradient(-45deg, white, ${randomColor})`;
                }
                else if (_this.gameMatrixBuffer[y][x] > 0 || _this.gameMatrix[y][x] > 0) {
                    var color = mainColor;
                    var shadowColor = secondColor;
                    //console.log(_this.gameMatrixBuffer[y][x])
                    if (_this.gameMatrixBuffer[y][x] == 1 || _this.gameMatrix[y][x] == 1) {
                        color = '#fd4868';
                        shadowColor = '#e80f15';
                    }
                    if (_this.gameMatrixBuffer[y][x] == 2 || _this.gameMatrix[y][x] == 2) {
                        color = '#fab619';
                        shadowColor = '#fb3807';
                    }
                    if (_this.gameMatrixBuffer[y][x] == 3 || _this.gameMatrix[y][x] == 3) {
                        color = '#fff833';
                        shadowColor = '#ff8307';
                    }
                    if (_this.gameMatrixBuffer[y][x] == 4 || _this.gameMatrix[y][x] == 4) {
                        color = '#6cf54e';
                        shadowColor = '#11750b';
                    }
                    if (_this.gameMatrixBuffer[y][x] == 5 || _this.gameMatrix[y][x] == 5) {
                        color = '#00ffff';
                        shadowColor = '#0054c6';
                    }
                    if (_this.gameMatrixBuffer[y][x] == 6 || _this.gameMatrix[y][x] == 6) {
                        color = '#ff35ff';
                        shadowColor = '#6b0c5f';
                    }
                    if (_this.gameMatrixBuffer[y][x] == 7 || _this.gameMatrix[y][x] == 7) {
                        color = 'hsl(244, 100%, 70%)';
                        shadowColor = 'hsl(244, 69%, 35%)';
                    }
                    element.style["background-image"] = `linear-gradient(135deg, ${color} 50%, ${shadowColor} 100%)`;
                    //element.style["background-color"] = color;
                }
                else if (_this.shadowFinderMatrix && _this.shadowFinderMatrix[y][x] > 0) {
                    element.style["background-image"] = `linear-gradient(${secondColor}, ${secondColor})`;
                    //element.style["background-color"] = secondColor;
                }
                else {
                    element.style["background-image"] = `linear-gradient(${bgColor}, ${bgColor})`;
            ...
            $("[nextpiece-block]").toArray().forEach(function (element) {
                ...
                if (_this.nextPieceMatrix[y][x] > 0)
                    element.style["background-image"] = `linear-gradient(-45deg, ${secondColor}, ${secondColor})`;
                else
                    element.style["background-image"] = `linear-gradient(${bgColor}, ${bgColor})`;
            });
        };
